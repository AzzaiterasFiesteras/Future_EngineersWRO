...
```
Es el núcleo del programa: se repite sin parar mientras el Arduino esté encendido, ejecutándose miles de veces por segundo.
- **Espera del pulsador.** Al inicio del bucle se comprueba si se ha pulsado el botón de arranque. Mientras no ocurra, la variable ´start´ permanece en ´false´  y el robot no hace nada más.
- **Condición de finalización.** Una vez iniciada la carrera, se comprueba si contadorGiros ha llegado a 12 (3 vueltas x 4 esquinas cada una). Si es así, se llama a ´terminarCarrera()´ y el programa se detiene definitivamente con un bucle infinito vacío.
- **Lectura de sensores.** En cada vuelta del bucle se leen los tres sensores de ultrasonido y también el ángulo actual mediante la función ´obtenerGrados()´.
- **Filtrado de lecturas.** Para evitar que una lectura puntual errónea provoque un giro incorrecto, el programa no reacciona a la primera detección, sino que usa dos contadores (´contador1´ y ´contador2´) que solo suben si la misma condición se cumple varias veces seguidas. Así se filtra el ruido típico de los sensores ultrasónicos.
- **Comportamiento ante un obstáculo:** cuando los contadores superan el umbral, el motor se detiene. Si es la primera esquina de toda la carrera, el programa decide él solo si el circuito se recorrerá en sentido horario o antihorario, según qué lado tenga más espacio libre. A continuación se llama a ´hacerGiro()´ para ejecutar el giro de 90 grados.
- **Comportamiento con el camino libre:** aquí es donde entra la cámara: antes de aplicar la corrección habitual, se consulta a ´detectarColor()´ si hay un bloque rojo o verde a la vista. Si detecta verde, el servo gira ligeramente a la izquierda; si detecta rojo, gira a la derecha. Solo si no detecta ninguno de los dos colores, el programa recurre a su lógica original: comparar el ángulo actual con el objetivo y corregir el rumbo con el giroscopio para mantener la trayectoria recta.

## Otras funciones 
`obtenerGrados()`: consulta al giroscopio y devuelve el ángulo de orientación actual del robot respecto a un punto de referencia fijo.
`prepararSensor()`: mantiene el programa en espera hasta que el nivel de calibración del giroscopio alcance su valor máximo, y a continuación establece el ángulo inicial como referencia de "línea recta".
`detectarColor()`: solicita a la cámara Pixy2 la lista de bloques de color detectados en ese instante, selecciona el de mayor tamaño entre los que coincidan con las firmas de rojo o verde, y devuelve un valor que identifica dicho color, o cero si no se detecta ninguno relevante.
`hacerGiro(grados)`: ejecuta físicamente el giro de la esquina, activando el motor y orientando el servomotor mientras compara continuamente el ángulo actual con el ángulo objetivo, hasta que la diferencia entre ambos sea suficientemente pequeña.
`terminarCarrera()`: detiene el proceso de las vueltas, hace avanzar el robot unos segundos adicionales para pasar completamente las 3 vueltas, y finalmente apaga el motor.
